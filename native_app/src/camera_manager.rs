...
impl NDKCamera {
    pub fn new() -> NDKCamera {
        ...
        unsafe { 
            ...
            ACameraManager_registerAvailabilityCallback(camera._camera_mgr, camera.get_manager_listener());
        }
        ...
        let metadata_obj = CameraMetadata::new();
        let metadata_obj_ref: &'static mut ACameraMetadata 
            = Box::leak(Box::new(metadata_obj));
        ...
        unsafe {
            ACameraManager_getCameraCharacteristics(
                ...
                c_str.as_ptr(),
                // TODO...
                // metadata_obj as *mut *mut ACameraMetadata
                0 as *mut *mut ACameraMetadata,
            );
        }
    ...
    pub fn on_camera_status_changed(&self, id: *const c_char, available: bool) { 
        // TODO...
        // if (valid_) {
    ...
    fn get_manager_listener(&self) -> *mut ACameraManager_AvailabilityCallbacks {
        let on_available: ACameraManager_AvailabilityCallback
            = Some(on_camera_available);

        let on_unavailable: ACameraManager_AvailabilityCallback
            = Some(on_camera_unavailable);

        let callbacks = ACameraManager_AvailabilityCallbacks {
            // TODO...
            // see: https://stackoverflow.com/questions/33929079/rust-ffi-passing-trait-object-as-context-to-call-callbacks-on
            context: (0 as *mut c_void),
            onCameraAvailable: on_available,
            onCameraUnavailable: on_unavailable
        };

        let static_ref: &'static mut ACameraManager_AvailabilityCallbacks 
            = Box::leak(Box::new(callbacks));
...
#[no_mangle]
extern "C" fn on_camera_available(ctx: *mut c_void, id: *const c_char) {
    // TODO...
    // let camera = Box::from_raw(ctx);
    // camera.on_camera_status_changed(id, true);
}

#[no_mangle]
extern "C" fn on_camera_unavailable(ctx: *mut c_void, id: *const c_char) {
    // TODO...
    // let camera = Box::from_raw(ctx);
    // camera.on_camera_status_changed(id, false);
}